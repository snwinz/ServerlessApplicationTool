...
public class InstrumentatorDefs implements CoverageInstrumentator {
    ...
    public String addCoverageStatementsInvocation(String line, String nameOfFunction) {
        String logLine = String.format("    %s.Payload = %s.Payload.replace('\\{','{\"funcDef\" : \"' + context.functionName + '_%s\",');%n" +
                "    console.log('%s' + JSON.parse(%s.Payload).funcDef + ' ');%n", nameOfFunction, nameOfFunction, currentID, functionInvocationMarker, nameOfFunction);
        currentID++;
        line = logLine + line;
        return line;
    }
