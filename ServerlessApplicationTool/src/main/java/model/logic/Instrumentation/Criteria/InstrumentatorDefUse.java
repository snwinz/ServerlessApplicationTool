...
public class InstrumentatorDefUse implements CoverageCriterion {
	...
	public String addDefOfReturns(String defVar, int line) {
		String logLine = String.format(
				"    %s.returnDef" + coverageAbbreviation + " = context.functionName + '_%s_line%s';%n"
						+ "    console.log('%s' + (%s.returnDef" + coverageAbbreviation + "));",
	...
	public String addUseOfEvents(String useVar, int line) {
		String logLine = String.format(
				"   if (event.funcDef" + coverageAbbreviation + " != undefined) {%n"
						+ "     console.log('%s' + event.funcDef" + coverageAbbreviation
						+ " + '_##' + context.functionName + '_line%s_%s');%n" + "   } "
						...
						+ " != undefined) {%n" + "					let funcDef" + coverageAbbreviation
						+ " = event.Records[0].dynamodb.NewImage.funcDef" + coverageAbbreviation + ".S;%n"
						+ "    				console.log('%s' + funcDef" + coverageAbbreviation
						+ " + '_##' + context.functionName + '_line%s_%s');%n" + "\t\t\t\t}%n\t\t\t}%n\t\t}%n\t}%n}%n ",
	...
	public String addDefOfDeletes(String deleteVar, int line) {
		String logLine = String.format(
				"if(%s.Key != undefined){%n" + "          %s.Item = %s.Key;%n" + "      }%n" + "      %s.Item.funcDel"
						...
						+ "      delete %s['Key'];%n" + "      console.log('%s' + %s.Item.funcDel"
						+ coverageAbbreviation + ".S);",
